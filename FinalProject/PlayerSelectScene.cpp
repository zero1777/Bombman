...
void PlayerSelectScene::Initialize() {
	...
	AddNewObject(new Engine::Image("PlayerSelect/P1_image.png", 150, halfH * 2 / 3, 310, 400));
	AddNewObject(new Engine::Image("PlayerSelect/P2_image.png", 800, halfH * 2 / 3, 310, 400));
	...
	btn->SetOnClickCallback(std::bind(&PlayerSelectScene::BackOnClick, this, 1));
...
void PlayerSelectScene::BackOnClick(int stage) {
...
void PlayerSelectScene::OnKeyDown(int keyCode) {
	IScene::OnKeyDown(keyCode);
	if (keyCode == ALLEGRO_KEY_TAB) {
		Engine::GameEngine::GetInstance().ChangeScene("PlayerSelectScene2");
	}
}